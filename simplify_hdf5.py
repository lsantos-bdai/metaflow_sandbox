...
def simplify_hdf5(input_path: str, output_path: str):
    ...
    with h5py.File(input_path, "r") as src, h5py.File(output_path, "w") as dst:
        ...
        # Create workspace_args with minimal valid YAML content
        dt = h5py.special_dtype(vlen=str)
        workspace_args_yaml = "{}"  # Minimal valid YAML
        dst.create_dataset("workspace_args", data=workspace_args_yaml, dtype=dt)
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, help="Output HDF5 file path (default: input file with _simplified suffix)"
    )
    ...
    if args.output:
        ...
        output_path = str(input_path.parent / f"{input_path.stem}_simplified{input_path.suffix}")
